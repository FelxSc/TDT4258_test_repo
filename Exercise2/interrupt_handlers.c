...
int playSample(uint8_t sound, int sampleNumber) // play sample returns the index of the next sample.
{
	...
	sleepmodeEM3();	// Enter sleepmodeEM3 when sound is finished
	stopTimer();		// Stop Timer1 when 
	return 0;
}
...
void __attribute__ ((interrupt)) GPIO_EVEN_IRQHandler()
{
	...
	if (isButtonPushed(2))
		...
		sleepmodeEM1();		// Enter sleepmodeEM1 for Timer1 interrupts
		startTimer();			// Start Timer1 for interrupts
